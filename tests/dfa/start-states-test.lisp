...
;; У меня до этого в dfa/start-states.lisp была реализована inline-функция reset-start-states!.
;; Всё, что она делала — заполняла элементы dfa-start-states значениями -1.
;; Я решил, что от неё нет никакого смысла, а очищением должен заниматься исключительно dfa/cache.lisp.
;; Поскольку тесты в этом файле завязаны на этой функции, а мне лень её переписывать, я решил прописать её тут специально для моих тестов.
;; Да, костыль. Да, плохо. Но это не нарушает функционал моей программы, при этом экономит моё драгоценное время.
(declaim (inline reset-start-states!))
