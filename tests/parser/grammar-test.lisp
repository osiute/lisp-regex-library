...
(defun test-grammar-quantifiers (assert-true-fn assert-equal-fn)
  ...
  (let ((ast (parse-regex "[0-9]{0,}")))
    (funcall assert-true-fn (ast-range-p ast) "узел ast-range")
    (funcall assert-equal-fn (ast-range-min ast) 0 "минимум диапазона = 0")
    (funcall assert-equal-fn (ast-range-max ast) nil "максимум диапазона = nil")
    (funcall assert-true-fn (ast-char-class-p (ast-range-child ast))
             "дочерний узел - символьный класс"))
  (let ((ast (parse-regex "[0-9]{0,0}")))
    (funcall assert-true-fn (ast-empty-p ast)
             "[0-9]{0,0}: ast-empty"))
  (let ((ast (parse-regex "(a)*")))
