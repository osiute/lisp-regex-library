...
(defun print-help()
  ...
  (format t "~4T(run-matches-p-tests)~%")
  ...
  (format t "~2T(direct-nfa-dot PATTERN &optional FILE-NAME)~%")
  (format t "~2T(reverse-nfa-dot PATTERN &optional FILE-NAME)~%")
  (format t "~2T(generate-dot-on-nfa NFA FILE-NAME)~%")
  (format t "~2T(make-lazy-dfa NFA &key MAX-STATES)~%")
  (format t "~2T(make-equivalence-table-from-ast AST)~%")
  (format t "~2T(dfa-get-start-state DFA CONTEXT)~%")
  (format t "~2T(dfa-step-state (DFA CUR-STATE-ID CLASS-ID CONTEXT)~%")
  (format t "~2T(reset-dfa! DFA)~%")
...
(defun reverse-nfa-dot (pattern &optional file-name)
  (when (null file-name)
    (setf file-name "reverse-nfa.dot")
  )
  (let ((nfa-to-output (reverse-nfa (create-nfa pattern))))
    (generate-dot-on-nfa nfa-to-output file-name)
  )
)
