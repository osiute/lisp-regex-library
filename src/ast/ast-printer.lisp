  ...
  (let ((connector (if last-p "└── " "├── "))
    ...
    (typecase node
      ...
      (ast-star
        ...
        (print-ast-node (ast-star-child node)
                        :prefix child-prefix :last-p t :stream stream))
      (ast-plus
        (format stream "AST-PLUS~%")
        (print-ast-node (ast-plus-child node)
                        :prefix child-prefix :last-p t :stream stream))
      (ast-question
        (format stream "AST-QUESTION~%")
        (print-ast-node (ast-question-child node)
                        :prefix child-prefix :last-p t :stream stream))
      (ast-range
        (format stream "AST-RANGE {~A,~A}~%"
                (or (ast-range-min node) "")
                (or (ast-range-max node) ""))
        (print-ast-node (ast-range-child node)
                        :prefix child-prefix :last-p t :stream stream))
      (ast-concat
      ...
      (ast-alt
        ...
        (print-ast-node (ast-alt-left node)
                        :prefix child-prefix :last-p nil :stream stream)
        (print-ast-node (ast-alt-right node)
                        :prefix child-prefix :last-p t :stream stream))
      (ast-empty
...
(defun print-ast (node &optional (stream *standard-output*))
  ...
  (typecase node
    (null
      (format stream "NIL~%"))
    (t
      (print-ast-node node :prefix "" :last-p t :stream stream)))
  node)
